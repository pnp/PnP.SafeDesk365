...
namespace SafeDesk365.SDK.Api.DeskAvailabilities.Upcoming {
    /// <summary>Builds and executes requests for operations under \api\deskAvailabilities\upcoming</summary>
    public class UpcomingRequestBuilder {
        ...
        public RequestInformation CreatePostRequestInformation(Action<PostQueryParameters> queryParameters = default, Action<IDictionary<string, string>> headers = default, IEnumerable<IRequestOption> options = default) {
            ...
            if (queryParameters != null) {
                var qParams = new PostQueryParameters();
                queryParameters.Invoke(qParams);
                qParams.AddQueryParameters(requestInfo.QueryParameters);
            }
            headers?.Invoke(requestInfo.Headers);
